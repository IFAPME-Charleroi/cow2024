...
async def text_to_speech(text):
    try:
        speak_task = asyncio.create_task(async_speak(text))
    except FileNotFoundError:
        print("Le programme espeak-ng n'a pas été trouvé. Veuillez l'installer.")
            ...
            with connection.cursor(pymysql.cursors.DictCursor) as cursor:
                ...
                if rows:
                    for row in rows:
                        ...
                        await text_to_speech("Vous entrez dans une zone dont la foule estimée est de " + total_personnes + " personnes.")
                        print("Ligne trouvée:")
                        print(row)
                else:
                    print(f"Aucune ligne trouvée pour le timestamp simulé: {simulated_time_str}")

            simulated_time += timedelta(minutes=2)
